"""Template 4: Cambria for title/headers, Arial for body. ALL CAPS section headers."""
...
CONTENT_SIZE = Pt(9.5)
SECTION_SIZE = Pt(12)
HEADER_ACCENT = RGBColor(0x4A, 0x7C, 0x59)  # Professional light green for name, subtitle, section headers
...
def build(context_path: str, output_path: str) -> None:
    ...
    if lines:
        ...
        r.font.color.rgb = HEADER_ACCENT
    ...
    if len(lines) > 1:
        ...
        for run in sub.runs:
            ...
            run.font.color.rgb = HEADER_ACCENT
    ...
    while i < len(lines):
        ...
        if s.upper() in headers or (s.isupper() and len(s) > 2 and s.isalpha()):
            ...
            in_profile = s.upper() in ("PROFILE", "SKILLS")
            in_work = s.upper() in ("WORK EXPERIENCE", "EXPERIENCE")
            if in_work:
                first_job = True
            if in_profile:
                first_skill = True
    ...
    for sec in doc.sections:
        ...
        sec.top_margin = Inches(0.7375)
        sec.bottom_margin = Inches(0.6875)
        sec.left_margin = Inches(0.5)
        sec.right_margin = Inches(0.375)

    doc.save(output_path)


CONFIG = {"id": 4, "name": "Template 4"}
