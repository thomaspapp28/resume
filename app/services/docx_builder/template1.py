"""Template 1: Blue accent, underlined sections."""
    ...
    while i < len(lines):
        ...
        if s.upper() in headers or (s.isupper() and len(s) > 2 and s.isalpha()):
            ...
            if not first_section:
                # Extra space between Summary and Skills
                space = Pt(20) if prev_section == "SUMMARY" and s.upper() == "SKILLS" else Pt(12)
                p.paragraph_format.space_before = space
            first_section = False
            prev_section = s.upper()
            in_cert = False
            r = p.add_run(s.upper())
            r.bold = True
